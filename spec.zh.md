...
- UIA sidecar 通信协议：stdio JSON Lines（docs/terminal/wt/sidecar.md）
- 无 Python 解释器依赖（用 PowerShell）
- 前端框架：vanilla TS（显示逻辑简单，不引框架；浏览器模式可直接跑 vite dev 用 Chrome DevTools 测试）
- **hook payload 契约（docs/agents/claude/hook.md）**：command 脚本转发、session_id 身份、marker 定位
