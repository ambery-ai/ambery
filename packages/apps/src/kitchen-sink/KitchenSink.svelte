<!-- Kitchen Sink——「全部组件与样式，一眼能看全吗」（契约见 docs/kitchen-sink.md）。
     版式：一种读法的两段（表面 → 规格）。每段至少一屏；整页是一篇文档、页面高度即内容高度，
     滚动条在窗口右缘；位移由实验 02 定下的引擎持有（算法抽在 reading-motion.ts，本页只接线）。
...
<script lang="ts">
  ...
  import { createMotion } from "./reading-motion";
  ...
  /** 实时读数（段号与引擎状态）：按帧循环的回调刷新 */
  let readout = $state("");

  /**
   * 阅读位移引擎：算法来自实验 02，抽在 reading-motion.ts 里——**本页只负责接线**，
   * 段元素取当前 DOM（内容重挂后引擎每次都按这个函数重取，不需要额外通知）。
   */
  const motion = createMotion(() => [...(page?.querySelectorAll<HTMLElement>(".tier") ?? [])], {
    onTick: (st) => {
      const text =
        "第 " + (st.section + 1) + " 段 · scrollY " + Math.round(window.scrollY) +
        " · 目标位 " + st.target + (st.gliding ? " · 过渡中" : "");
  ...
  onMount(() => {
    ...
    motion.start();
    // 调试入口：控制台里读实时状态（`__motion.state()`）
    (window as unknown as { __motion?: unknown }).__motion = motion;
    return () => {
