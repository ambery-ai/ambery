...
> 本文档约束 `docs/*.md` 与 `concepts.md` 的职责边界与内容准入。它只管这两处；`reports/`（调研）另归其管，不在本文档范围。所有 `docs/*.md` 与 `concepts.md` 改动动手前必须先通读本文件。
...
- `access-protocol.md` — Ambery Protocol 契约：外部软件如何成为 Source，事件如何进出

#### 终端

- `docs/terminal/terminal-adapter.md` — 终端访问抽象：定位/读取/解除定位接口、各终端实现、config 字段与分层接入模型（L1–L3 / M1–M3）
- `docs/terminal/wt/sidecar.md` — WtAdapter 进程协议（wt 终端适配器的独立进程：stdio JSONL、命令集与生命周期）
- `docs/terminal/timer.md` — 兜底巡逻扫描调度、错峰算法与扫描动作应用点

#### Agent

- `docs/agents/claude/hook.md` — 真实 Claude Code hook 契约：事件分层、marker 定位、启动扫描、安装
- `docs/agents/filter.md` — 终端文本过滤策略与结构理解数据类型
...
- `platform-primitives.md` — Ambery 自身的平台能力层（自身窗口的定位/聚焦/桌面处理）：接口与各平台实现
...
- `ui-composition.md` — UI 代码分层与组合：宿主 / Surface / widget 分层、widget 各层、样式组合、组件原则
...
- `roadmap.md` — 通向 0.1.0 的发布阶梯：每档主题与验收；版本边界的唯一归属
- `post-0.1.0.md` — 0.1.0 后能力路线图：每个后续能力一段简述
...
- `docs/` 留在根、不拆：接入类文档住在 `docs/`（`docs/terminal/`、`docs/agents/`、`docs/cron.md`），不进 packages。
- 同组的父文档不承担子文档的登记：组里有哪些文档由责任地图陈述一次，不在父文档引言里逐一列举。
...
- **版本信息**——docs 不写任何版本号或版本范围（如"X 属 0.1.0 前/后"）。版本边界由发布阶梯（`docs/roadmap.md`）定义、且仅由它定义；单个能力文档不临时自定版本归属。
- **状态标注**——docs 不写易变的状态（当前契约 / 待落地 / 未决等）。被取代的历史方案删除或在原文标注历史，不以维护中的状态字段表达。
- **不引用内部 issue**——docs 不引用内部问题编号（#N、issue-xxx、issues #N）：文档只陈述当前状态与契约，不把内部问题编号当证据或锚点；外部上游引用（如上游 issue / discussion 编号）允许。
- **调研与论证过程**——该进 `reports/`，docs 只记录已收敛的结论；调研的来龙去脉不属于设计契约。
- **单次会话的过程回顾**（grill 回顾等）——该进 `drafts/`，不是设计契约。
- **待落地/未决实现清单**——执行项进开发 ticket（本轮修复），未来项进 `docs/post-0.1.0.md`（路线图）；不沉淀进文档冒充实现依据。
...
契约文档用 `## 原则` 段声明其长期有效的规则，位置在文档顶部的范围注记之后、机制各节之前。这一段不是必需的：规则已由文档自身的数据模型、字段表、接口或图示承载的文档（格式、坐标契约、流程图）不加此段，缺这一段不算缺口。

- 一条原则一个引用段，形式为 `> **<名字>** —— <规则>`；原则之间空一行。
- 第一条原则是本文的范围：本文定义什么，不定义什么、归哪份文档。
- 原则只写长期规则与边界，不写机制步骤；需要细节的原则指向拥有该细节的节或文档。
- 原则以规则命名，不用编号、版本或状态。
