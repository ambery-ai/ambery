...
//   case-runner 进程主体内嵌 core 并拉起本 TS 测试进程（overseer-case frontend）；
//   测试经 RemoteBridge 连内嵌 core（createBridge 无 __TAURI_INTERNALS__ 自动选中，
//   HTTP+WS 真链路）；本文件不拦截任何 Tauri API，只提供：
//   - 端口接线（__OVERSEER_PORT__，必须先于 src 模块导入生效）
//   - core 就绪等待（serve 与 vitest 并发启动，测试可能先跑）
//   - 沙盒 effect.jsonl 读取（OVERSEER_STORAGE_DIR 由 case-runner 注入）
...
(globalThis as Record<string, unknown>).__OVERSEER_PORT__ =
  process.env.OVERSEER_PORT ?? "47600";
// RemoteBridge 的 new WebSocket(...) 走全局构造器——node 下以 ws 包替换（realm 冲突规避）
(globalThis as Record<string, unknown>).WebSocket = WebSocket;

export function coreBase(): string {
  return `http://127.0.0.1:${process.env.OVERSEER_PORT ?? "47600"}`;
}
...
export async function waitCore(ms = 30000): Promise<void> {
  ...
  for (;;) {
    ...
    if (Date.now() - t0 > ms) throw new Error("core 未就绪（overseer-case frontend 未完成装配）");
    await new Promise((r) => setTimeout(r, 200));
  }
}

/** 沙盒 effect.jsonl（storage 目录经 OVERSEER_STORAGE_DIR 由 case-runner 注入） */
export function readEffects(): string {
  try {
    return readFileSync(join(process.env.OVERSEER_STORAGE_DIR!, "effect.jsonl"), "utf8");
  } catch {
    return "";
  }
}
