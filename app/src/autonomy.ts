...
export class Autonomy {
  ...
  /** 默认推导（优先级：notify > processing > idle）；key 在两池并集解析 */
  deriveDefault(state: TopState): Expression {
    ...
    // key 不再存在时回落内置默认（当前状态 key 消失 → 回落默认状态）
    return (
  ...
  setAutonomy(args: { face?: string; motion?: Motion; ttlMs?: number; once?: boolean }) {
    ...
    // 省略 ttlMs 的默认：config 下发值（默认 60000）；5000 仅为投影缺失兜底
