...
| `spec.md` | 技术选型、结构决定、取舍 | 运行机制（→ `docs/`）、概念定义（→ `concepts.md`） |
| `reports/` | 调研结论：证据 + 结论 | 过程记录、契约正文 |
| 根 `README.md` | 项目入口：是什么、quickstart、平台矩阵 | 架构细节（→ `docs/`） |
| `docs/roadmap.md` | 发布阶梯：通向 0.1.0 的主题与验收 | ticket 级规划、契约正文 |
...
- 引用概念或章节一律用名字，绝不用位置编号。列表一旦重排，编号就会漂，所有编号引用随之失效。
...
- 代码一律以模块、类型、函数、工具或 IPC 的名字指称，不写源码文件路径与行号——文件一改名、一搬动就腐烂，而名字仍能找到定义。路径本身即契约的除外（数据产物如 `memory/cards/<id>.card.json`、`config.json`，以及文档路径）。
- 示例代码与实际实现一致，不得误导。
...
- 手抄仓库文件树：仓库自身（workspace 清单与目录）是权威来源，且不随文档改动而变；文档用 crate、模块、类型的名字指称。数据产物布局（storage 目录、config 位置）是契约内容，保留。
