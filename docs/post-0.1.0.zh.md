...
Card 目前出现/关闭为瞬时、仅随 View 移动；整体动效补齐——入场/离场、沿 direction 的弹出、悬停/强调、与 pet 自主动作一致的状态驱动转场——为后续能力。正式设计时需要独立确定：动作语汇及 card 是否与 pet 共用 Motion 注册表还是独立一套；按 card 类型的动画映射与差异；用户拖拽或快速 effect 更新重渲染时动画的中断/取消语义；以及动画归属（Surface 显示层的本地 `_meta` 管理状态、不进入 Agent 投影，还是成为 card spec 的一部分）。
